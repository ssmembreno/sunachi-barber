      ...
      <div class="relative w-full max-w-lg rounded-2xl border border-gray-200 bg-white p-5 shadow-xl dark:border-gray-800 dark:bg-gray-900">
        <div class="flex items-start justify-between gap-3">
          <div>
            ...
            <p v-if="selectedEvent?.extendedProps?.price" class="mt-1 text-xs text-gray-500 dark:text-gray-400">
              Total a Pagar:
              <span class="font-medium text-gray-900 dark:text-gray-100">{{ selectedEvent.extendedProps.price }} Lps</span>
            </p>
          </div>
        ...
        <div class="mt-5 grid grid-cols-2 gap-3">
          ...
          <button
            v-if="['pending', 'confirmed'].includes(selectedEvent?.extendedProps?.status)"
            type="button"
            class="col-span-2 rounded-xl border border-red-500 bg-white px-4 py-2 text-sm font-medium text-red-600 hover:bg-red-50 dark:border-red-900 dark:bg-gray-900 dark:text-red-500 dark:hover:bg-red-900/30 transition-colors"
